...
const App: React.FC = () => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (url.includes('youtube') || url.includes('youtu.be')) {
        ...
        platform = 'generic';
        detectedTitle = url.split('/').pop() || "Direct Video File";
      }
  ...
  const handleDownload = async (cleanCopyright: boolean = false) => {
    ...
    try {
      ...
      // LOGIC: If the URL is NOT a direct file (like youtube.com), browsers CANNOT download it directly due to CORS.
      // In a real production app, 'downloadSource' would be replaced by a backend API response (e.g., api.mysite.com/get-stream?url=...)
      // Here, we check if it looks like a direct file. If not, we fall back to a sample for the DEMO.
      ...
      if (!isDirectFile) {
         ...
         // Fallback to high-quality sample video if we can't scrape the real one client-side
      ...
      // IMPORTANT: Rename the file to match the VIDEO TITLE so it feels like the correct video
      const safeTitle = videoData?.title.replace(/[^a-z0-9]/gi, '_').substring(0, 50) || "video";
      const prefix = cleanCopyright ? "StreamSage_Clean_" : "StreamSage_";
      a.download = `${prefix}${safeTitle}_${Date.now()}.mp4`;
